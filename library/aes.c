                           ...
                           unsigned int keybits)
{
    ...
    for (unsigned int i = 0; i < 10; i++, rk_m += 4, rk_s += 4){
        ...
        uint32_t temp_rot_masked =  (uint32_t) MBEDTLS_BYTE_1(rk_m[3]) 
                                  | (uint32_t) MBEDTLS_BYTE_2(rk_m[3]) << 8
                                  | (uint32_t) MBEDTLS_BYTE_3(rk_m[3]) << 16
                                  | (uint32_t) MBEDTLS_BYTE_0(rk_m[3]) << 24;
        uint32_t temp_rot_mask = (uint32_t) MBEDTLS_BYTE_1(rk_s[3]) 
                                  | (uint32_t) MBEDTLS_BYTE_2(rk_s[3]) << 8
        ...
        uint32_t next_word_mask = rk_s[0] ^ temp_sub_mask;
     ...
     */
    {
        ...
        for (i = 0; i < num_tests << 1; i++) {
            ...
            if (ret == MBEDTLS_ERR_PLATFORM_FEATURE_UNSUPPORTED && keybits == 192) {
                ...
                mbedtls_printf("MBEDTLS_ERR_PLATFORM_FEATURE_UNSUPPORTED failed");
                goto exit;
            }
